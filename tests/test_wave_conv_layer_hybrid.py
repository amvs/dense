...
class TestFilterInitialization:
    ...
    def test_nearest_neighbor_initialization(self):
        """Verify upsampled filters are initialized with nearest neighbor"""
        layer = WaveConvLayerHybrid(
            J=3, L=4, A=1, T=4, M=16, N=16,
        ...
        # Check that the upsampled filter is 4x larger
        # up_real is (1, 4, 1, 16, 16), base_real is (1, 4, 1, 4, 4)
        assert up_real_j2.shape[-1] == 4 * base_real.shape[-1]
        # check that downsampled version of big filter
        # matches original filter
        assert torch.allclose(base_real, resize(up_real_j2, size=(4, 4), mode="nearest"))
