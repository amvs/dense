...
class WaveConvLayer(nn.Module):
    def __init__(
        ...
        if filters is None:
            # Initialize randomly
            real = torch.randn(param_nc, param_J, param_L, param_A, M, N)
            imag = torch.randn(param_nc, param_J, param_L, param_A, M, N)
            base_filters = torch.complex(real, imag)
        else:
            # Use precomputed filters (assume they're already in reduced form)
            expected_shape = (param_nc, param_J, param_L, param_A, M, N)
            assert filters.shape == expected_shape, \
    ...
    def get_full_filters(self):
        ...
        if self.share_scales:
            filters = filters.expand(-1, self.J, -1, -1, -1, -1)
        if self.share_rotations:
            filters = filters.expand(-1, -1, self.L, -1, -1, -1)
        if self.share_phases:
            filters = filters.expand(-1, -1, -1, self.A, -1, -1)
    ...
    def forward(self, x):
        ...
        hatxpsi_bc = hatpsi_la * hatx_c.view(nb, num_channels, 1, 1, 1, self.M, self.N)  # (nb, nc, J, L, A, M, N)
