...
def rotation_matrix(angle_degrees, batch_size, device):
    ...
    rot_mat = torch.tensor(
        [[cos_val, -sin_val, 0], [sin_val, cos_val, 0]], device=device
    )
...
def periodic_rotate(spatial_filters: torch.Tensor, angle_degrees: float):
    ...
    real_padded = F.pad(
        f_hat_shifted.real, (pad_w, pad_w, pad_h, pad_h), mode="circular"
    )
    imag_padded = F.pad(
        f_hat_shifted.imag, (pad_w, pad_w, pad_h, pad_h), mode="circular"
    )
...
def apply_phase_shifts(filters: torch.Tensor, A: int) -> torch.Tensor:
    ...
    expanded_filters = (
        filters.unsqueeze(-3).repeat(1, 1, A, 1, 1).clone()
    )  # Clone to avoid in-place operations
    for a in range(A):
        i = torch.complex(
            torch.tensor(0.0, device=filters.device),
            torch.tensor(1.0, device=filters.device),
        )
